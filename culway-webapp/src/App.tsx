...
const ALL_ROUTES: RouteCard[] = [
  { id: "rt1", title: "Москва: центр и виды", short: "РАБОЧИЙ МАРШРУТ. Красная площадь, Зарядье, Киевская набережная", rating: 4.85, category: "Творчество и вдохновение", image: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/01/Moscow_July_2011-16.jpg/960px-Moscow_July_2011-16.jpg" },
  { id: "rt2", title: "Сочи: горы и морской бриз", short: "Тропа здоровья, виды и хмели-сунели", rating: 4.82, category: "Движение и энергия", image: "https://upload.wikimedia.org/wikipedia/commons/thumb/3/32/Аэрофотоснимок_пляжа_%22Маяк%22%2C_район_морпорта%2C_г._Сочи.jpg/1600px-Аэрофотоснимок_пляжа_%22Маяк%22%2C_район_морпорта%2C_г._Сочи.jpg" },
  { id: "rt3", title: "Казань: вкус жизни", short: "Чак-чак, плов и гастро-рынки", rating: 4.71, category: "Вкус жизни", image: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b9/Kazan_Kremlin_exterior_view_08-2016_img3.jpg/2560px-Kazan_Kremlin_exterior_view_08-2016_img3.jpg" },
  { id: "rt4", title: "Санкт-Петербург: научные открытия", short: "Планетарий, ИТМО, кунсткамера", rating: 4.76, category: "Знания и открытия", image: "https://upload.wikimedia.org/wikipedia/commons/c/c4/ITMO_University_Main_Campus_October_2024.jpg" },
  { id: "rt5", title: "Москва by night", short: "Арт-кварталы и вечерние концерты", rating: 4.80, category: "Звуки и ритмы", image: "https://upload.wikimedia.org/wikipedia/commons/a/a9/Moscow_05-2017_img39_Patriarshiy_Pond.jpg" },
];

const ROUTE_TITLES: Record<string, string> =
  Object.fromEntries(ALL_ROUTES.map(r => [r.id, r.title]));
const routeTitle = (rid: string) => ROUTE_TITLES[rid] ?? rid; // fallback на id
...
type Tab = "home" | "done" | "kuly" | "promos" | "rating" | "vision"; // нижняя навигация
...
const [profile, setProfile] = useLocalStorage<Profile>("cw.profile", {
  ...
  const finishPrefs = () => {
    // если выбрано что-то — сохраняем массив категорий, иначе "all"
    setFilter(prefs.length ? [...prefs] : "all");
    setStep("app");
  };
  ...
  return (
    ...
    >
      <Header
        onMapClick={() => { setStep("app"); setActiveTab("vision"); }}
  ...
  return (
    <Drawer onClose={onClose}>
      ...
      <div className="mt-3 space-y-2 text-sm" style={{ color: palette.coffee }}>
        ...
        <p><b>Номер телефона</b> +79854525258</p>
      ...
      <div className="mt-6 flex flex-col items-center gap-5">
        ...
        <div className="flex flex-col gap-3">
          ...
          style={{
            ...
            border: `3px solid ${palette.caramel}`,
            borderRadius: 12,
            padding: "10px 14px",
          ...
          <button disabled={reqPending} className="rounded-md px-4 py-2 basis-full"
...
/** ================== Экран: Лента + запуск маршрута (с модалкой фильтров) ================== */
/** ================== Экран: Лента + запуск маршрута (с модалкой фильтров) ================== */
function HomeScreen({
  ...
  const toggleTempCat = (cat: Category) => {
    setTempCats(prev => {
      const i = prev.indexOf(cat);
      if (i >= 0) { const copy = [...prev]; copy.splice(i,1); return copy; }
      return [...prev, cat];
    });
  };
  const clearTemp = () => setTempCats([]);
  const applyTemp = () => {
    setFilter(tempCats.length ? tempCats : "all");
    setFilterOpen(false);
  };

  // фильтрация ленты по активным категориям
  ...
  return (
    <div className="px-5 py-6">
      ...
      {/* Кнопка «Фильтр» — открывает модалку */}
      <div className="mt-3">
        <button
          className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-xl"
          style={{ background: palette.coffee, color: "#f6eae0" }}
          onClick={() => {
            setTempCats(filter === "all" ? [] : [...(filter as Category[])]);
            setFilterOpen(true);
          }}
        >
          {hasActiveFilter ? "Фильтр (выбрано)" : "Фильтр"}
        ...
        {filtered.map((r)=>(
          <div key={r.id} className="rounded-2xl p-4"
            ...
            <div className="flex gap-4">
              <div className="h-28 w-28 flex-shrink-0 rounded-xl overflow-hidden bg-neutral-300">
                {r.image ? (
                  <img
                    src={r.image}
                    alt={r.title}
      ...
      {detail && (
        <Drawer onClose={()=>setDetail(null)}>
          {/* <<< вот тут теперь картинка карточки >>> */}
          {detail.image ? (
            <div className="mb-4 h-48 w-full rounded-2xl overflow-hidden bg-neutral-300">
              <img
                src={detail.image}
                alt={detail.title}
                loading="lazy"
      ...
      {/* Модалка фильтров */}
      {filterOpen && (
        <Drawer onClose={()=>setFilterOpen(false)}>
          <div className="text-2xl font-semibold" style={{ color: palette.coffee }}>
            Фильтры
          </div>

          <div className="mt-3">
            <FilterChip active={tempCats.length===0} onClick={clearTemp}>Все</FilterChip>
          </div>
      ...
      <div className="mt-4 space-y-3">
        ...
        {completed.map((rid)=>(
          <div key={rid} className="rounded-xl p-4" style={{ background: palette.cream, color: palette.coffee }}>
            Маршрут <b>{routeTitle(rid)}</b> завершён ✅
          </div>
        ))}
      </div>
    </div>
...
/** ================== Экран: Объектив (распознавание по фото) ================== */
type VisionResult = {
  title: string;
  summary: string;
  tags?: string[];
...
function VisionScreen() {
  ...
  const [resu, setResu] = useState<VisionResult | null>(null);
  const [error, setError] = useState<string>("");

  const onPick = (e: React.ChangeEvent<HTMLInputElement>)=>{
    const f = e.target.files?.[0]; if (!f) return;
    const r = new FileReader();
    r.onload = ()=> { setPreview(String(r.result)); setResu(null); setError(""); };
    r.readAsDataURL(f);
  };

  async function analyzeImage(dataUrl: string): Promise<VisionResult> {
    // 1) Пытаемся сходить на твой бэкенд (если есть)
    try {
      ...
      if (resp.ok) {
        ...
        // ожидаем формат { title, summary, tags? }
        if (json?.title && json?.summary) return json as VisionResult;
      }
    } catch (_) { /* упадём в демо */ }

    // 2) Демо-ответ (локально, без сервера)
    return {
      "title": "Храм Василия Блаженного",
      "summary": "Православный храм XVI века на Красной площади в Москве. Построен при Иване Грозном.",
      "tags": ["архитектура", "достопримечательность", "Москва"]
    };
  }

  const onAnalyze = async ()=>{
    if (!preview) { setError("Сначала выберите фото."); return; }
    setLoading(true); setError(""); setResu(null);
  ...
  return (
    <div className="px-5 py-6">
      <BackTitle title="Объектив" />
      <div className="mt-4 space-y-3">
        <div className="rounded-xl p-3" style={{ background: palette.cream, color: palette.coffee }}>
          <div className="text-lg font-semibold mb-2">Фото объекта</div>
          <div className="flex gap-2 items-center">
            ...
            <button
              ...
              className="rounded-xl px-4 py-2 w-full sm:w-auto"
              style={{
                background: palette.caramel,
                color: "#fff",
                border: "3px solid #7b4b31",   // ← рамка
              }}
            >
              📷 Выбрать файл
            </button>
          </div>
          <div className="mt-3 h-48 w-full rounded-xl overflow-hidden bg-neutral-200 flex items-center justify-center">
            {preview ? <img src={preview} alt="preview" className="h-full w-full object-cover"/> : <span className="opacity-60">Предпросмотр</span>}
          </div>
        ...
        {resu && (
          <div className="rounded-xl p-4" style={{ background: "#fff4cc", color: "#533" }}>
            <div className="text-lg font-semibold">{resu.title}</div>
            <div className="mt-1 text-sm">{resu.summary}</div>
            {resu.tags?.length ? (
              <div className="mt-2 flex flex-wrap gap-2">
                {resu.tags.map((t,i)=>(
                  <span key={i} className="rounded-md px-2 py-1 text-xs" style={{ background:"#F1F5F9", color:"#0f172a" }}>
                    #{t}
                  </span>
                ))}
              </div>
...
function UploadFinishPhotoModalInner({ routeId, onEdit, onClose, onPhotoSaved }: { routeId:string; onClose:()=>void; onPhotoSaved:(ph:KulaPhoto, gavePromo:boolean)=>void; onEdit: () => void; }) {
  ...
  return (
    <Drawer onClose={onClose}>
      ...
      <div className="mt-1 text-sm opacity-80">Загрузите фото из финиша маршрута для получения бонуса.</div>
      <div className="mt-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          />
          <button
            className="rounded-md px-4 py-2 basis-full" // ← займёт всю строку ниже
            style={{ background: palette.caramel, color:"#fff" }}
            onClick={onSave}
          >
            Сохранить
